...
for q in range (1,Max_iterations):
    ...
    mdb.models['Model-%d' %(q)].sketches['__profile__'].rectangle(point1=(-0.095, 0.095), 
        point2=(0.095, -0.095))
    ...
    while (num_incl < max_incl):
        random_x=random.uniform(-0.17575/2, 0.17575/2)  #generate random x_coordinate within RVE
        random_y=random.uniform(-0.17575/2, 0.17575/2)  #generate random y_coordinate within RVE
    ...
    for i in range(max_incl):    
        ...
        mdb.models['Model-%d' %(q)].parts['Part-1'].PartitionFaceBySketch(faces=
            mdb.models['Model-%d' %(q)].parts['Part-1'].faces.findAt(((0.095, 
            0.095, 0.0), (0.0, 0.0, 1.0)), ), sketch=mdb.models['Model-%d' %(q)].sketches['__profile__'])
    ...
    mdb.models['Model-%d' %(q)].parts['Part-1'].SectionAssignment(offset=0.0, 
        ...
        faces=mdb.models['Model-%d' %(q)].parts['Part-1'].faces.findAt(((0.095, 
            0.095, 0.0), (0.0, 0.0, 1.0)), )), sectionName='Matrix', 
    ...
    mdb.models['Model-%d' %(q)].DisplacementBC(amplitude=UNSET, createStepName='Step-1', 
        ...
        ((-0.25, -0.5, 0.0), ), )), u1=UNSET, u2=0.0, ur3=UNSET)
    mdb.models['Model-%d' %(q)].DisplacementBC(amplitude=UNSET, createStepName='Step-1', 
        distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, name=
        'BC-2', region=Region(
        edges=mdb.models['Model-%d' %(q)].rootAssembly.instances['Part-1-1'].edges.findAt(
        ((-0.5, 0.25, 0.0), ), )), u1=0.0, u2=UNSET, ur3=UNSET)
    mdb.models['Model-%d' %(q)].DisplacementBC(amplitude=UNSET, createStepName='Step-1', 
        distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, name=
        'BC-3', region=Region(
        edges=mdb.models['Model-%d' %(q)].rootAssembly.instances['Part-1-1'].edges.findAt(
        ((-0.25, 0.5, 0.0), ), )), u1=UNSET, u2=1.0, ur3=UNSET)
    ...
    for i in range (num_incl):
        mdb.models['Model-%d' %(q)].rootAssembly.setElementType(elemTypes=(ElemType(
            elemCode=CPS4T, elemLibrary=STANDARD), ElemType(elemCode=CPS4T, 
    ...
    mdb.models['Model-%d' %(q)].rootAssembly.setElementType(elemTypes=(ElemType(
        elemCode=CPS4T, elemLibrary=STANDARD), ElemType(elemCode=CPS4T, 
        elemLibrary=STANDARD)),  regions=(
        mdb.models['Model-%d' %(q)].rootAssembly.instances['Part-1-1'].faces.findAt(((0.05, 
            0.05, 0.0), )), ))
    ...
    mdb.models['Model-%d' %(q)].rootAssembly.Set(edges=
        ...
        -0.095, -0.095, 0.0), )), name='BottomEdge')
    mdb.models['Model-%d' %(q)].rootAssembly.Set(edges=
        mdb.models['Model-%d' %(q)].rootAssembly.instances['Part-1-1'].edges.findAt(((
        -0.095, 0.095, 0.0), )), name='TopEdge')
