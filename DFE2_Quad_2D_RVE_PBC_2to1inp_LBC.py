...
MacroInpName = 'MACRO_1el.inp' # Name of macroscale input file
...
for i in range(Nele):
    ...
    for j in range(4): # 4 GPs
        ...
        for k in dummy:
            print('*Nset, nset=Ele'+str(i+1)+'-RVE'+str(j+1)+'-node'+str(k+1)+', instance=Ele'+str(i+1)+'-RVE'+str(j+1),file=Sets)
            print(str(k+1),file=Sets)

            for dof in [10]:
                print('** Constraint: Ele'+str(i+1)+'-RVE'+str(j+1)+'-node'+str(k+1)+'-DOF'+str(dof+1),file=Eqns)
                print('*Equation',file=Eqns)
                print('5',file=Eqns)
                print('Ele'+str(i+1)+'-RVE'+str(j+1)+'-node'+str(k+1)+', '+str(dof+1)+', -1.0',file=Eqns)
                for m in range(4):
                    print('Ele'+str(i+1)+'-N'+str(m+1)+', '+str(dof+1)+', '+str(Shape_fn[m]+N_GloDeriv[m][0]*RVENodalCoord[k][0]+N_GloDeriv[m][1]*RVENodalCoord[k][1]),file=Eqns)
